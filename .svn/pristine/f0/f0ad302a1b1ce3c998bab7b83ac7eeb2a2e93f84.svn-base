...
//            var countSrc = $('.nav-list .count>a').attr('name'), //count.html
        ...
        if(role == '医院财务员'){
          $('.nav-list .finance').show();
          var moneyUrl = $('.nav-list .finance>a').attr('name')
          $('#iframe').attr('src',moneyUrl).ready();
...
//参数： userId=b9ef4e97c057feca766d63c59e382b55 用户id userId
    ...
    $(".statistics").on("click",function(){
      // $(".statisticsList a span").css("color","#333")
      if($(".statisticsList").css("display")=="none"){
    ...
    $(".user").on("click",function(){
      if(!$(this).hasClass('statistics')){
        $(".statistics").find(".fa-angle-down").addClass("fa-angle-right").removeClass("fa-angle-down")
        $(".statisticsList").hide()
      }
    })
    $(".statisticsList a").on('click',function(event){
      event.stopPropagation()
      $(this).find("i").css("background","url(images/Oval.png) no-repeat")
      $(this).siblings().find("i").css("background","url(images/Oval_black.png) no-repeat")
      $(this).find("span").css("color","#2bd1c7")
